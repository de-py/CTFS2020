	global	_start
	section	.text

_start:
	call	main
	mov	rdi, rax
	mov	rax, 60 ; exit
	syscall ; exit

main:
	push	rbp
	mov	rbp, rsp
	sub	rsp, 0x50 ; 
	mov	rax, 0x29 ; socket
	mov	rdi, 0x02 ; int family
	mov	rsi, 0x01 ; int type
	mov	rdx, 0x00 ; int protocol
	syscall ; socket
	mov	QWORD [rbp-0x10], rax
	mov	rax, 0x36 ; setsockopt
	mov	rdi, QWORD [rbp-0x10] ; int fd
	mov	rsi, 0x01 ; int level
	mov	rdx, 0x02 ; int optname
	mov	QWORD [rbp-0x50], 0x01 
	lea	r10, [rbp-0x50] char *optval
	mov	r8, 0x08 int *optlen
	syscall ; setsockopt
	mov	rax, 0x31 ; bind 
	mov	rdi, QWORD [rbp-0x10] ; int fd
	push	0x00
	mov	bx, 0x0539
	xchg	bh, bl
	shl	ebx, 0x10
	mov	bx, 0x02
	push	rbx
	mov	rsi, rsp ; struct sockaddr *umyaddr
	add	rsp, 0x10
	mov	rdx, 0x10 ; int addrlen
	syscall ; bind
	mov	rax, 0x32 ; listen
	mov	rsi, 0x01 ; int fd
	syscall ; listen
	mov	rax, 0x2b ; accept
	mov	rsi, 0x00
	mov	rdx, 0x00
	syscall ; accept
	mov	QWORD [rbp-0x08], rax
	mov	rbx, 0x6170207265746e45
	mov	QWORD [rbp-0x50], rbx
	mov	rbx, 0x203a64726f777373
	mov	QWORD [rbp-0x48], rbx
	mov	rax, 0x01 ; write
	mov	rdi, QWORD [rbp-0x08]
	lea	rsi, [rbp-0x50]
	mov	rdx, 0x10
	syscall ; write
	mov	rax, 0x00 ; read
	lea	rsi, [rbp-0x50]
	mov	rdx, 0x40
	syscall ; read
	cmp	rax, 0x11
	jne	label5
	lea	rax, [rbp-0x50]
	cmp	BYTE [rax+16], 0x0a
	jne	label5
	mov	BYTE [rax+16], 0x00
	jmp	label2
label1:
	xor	BYTE [rax], 0x69
	inc	rax
label2:
	cmp	BYTE [rax], 0x00
	jne	label1
	mov	rax, 0x360c1f0605360c1e
	cmp	QWORD [rbp-0x50], rax
	jne	label5
	mov	rax, 0x0c0c10361b041a08
	cmp	QWORD [rbp-0x48], rax
	jne	label5
	mov	rdi, dat
	lea	rax, [rbp-0x50]
	mov	rbx, 0x00
	mov	rcx, 0x00
	mov	rdx, 0x00
	jmp	label4
label3:
	mov	dl, BYTE [rdi]
	xor	dl, BYTE [rax+rbx]
	mov	BYTE [rdi], dl
	inc	rdi
	inc	rbx
	inc	rcx
	cmp	rbx, 0x10
	jne	label4
	mov	rbx, 0x00
label4:
	cmp	rcx, len
	jne	label3
	mov	rax, 0x01 ; write
	mov	rdi, QWORD [rbp-0x08]
	mov	rsi, dat
	mov	rdx, len
	syscall ; write
	jmp	label6
label5:
	mov	rbx, 0x0a2165706f4e
	mov	QWORD [rbp-0x50], rbx
	mov	rax, 0x01 ; write
	mov	rdi, QWORD [rbp-0x08]
	lea	rsi, [rbp-0x50]
	mov	rdx, 0x06
	syscall ; write
label6:
	mov	rax, 0x03 ; close
	mov	rdi, QWORD [rbp-0x08]
	syscall ; close
	mov	rax, 0x03
	mov	rdi, QWORD [rbp-0x10]
	syscall ; close
	mov	rax, 0x00 
	leave
	ret

	section	.data

	dat	db	
	len	equ	$-dat

